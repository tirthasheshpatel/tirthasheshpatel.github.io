...
`Normal Form (Matrix Form)`: List what payoffs each player gets as a function of their actions.
  - It is *as if* all players move simultaneously.
  - Strategies encode many things.

`Extensive form` includes the timing of the moves (actions).
  - Players move sequentially (represented as a tree!).
  - Keeps track of what each player knows when he/she makes each decision.

{: .box-warning}
**Note**: In games represented by a `Normal Form`, the players don't have the knowledge of what other players have chosen.
...
**Representation**: Finite, $n$ person `normal form` game: $\langle N, A, u \rangle$
  ...
  - `Utility function` is a function mapping actions of each player to their utilities. Hence, it takes as input an `action profile` $a$ and outputs a utility vector of size $n$ representing the utility of each player for that action profile.
    - $u: A \to \mathbb{R}^n$

{: .box-warning}
**Note**: The utility of the players doesn't only depend on their actions but also on the actions of other players.

### More Games

`Prisoners Dilemma` is any game that has the following form.
...
Example 2: Game of Rock-Paper-Scissors! Whoever wins gets $1$ other gets $-1$ and if it ties, both get $0$. This is also a game of pure competition and a zero-sum game.

{: .box-error}
**Exercise**: Try to find more games of pure competition.

`Games of co-operation`: Players have exactly the same interests.
...
Let's play a game where you will have to rate a model on a scale of 1-100 and you will win if you are closest to 2/3'rd of the average of ratings of all the other players.

Let's try to reason through this game and choose our best action:
  - Suppose the average is $X$.
  - This means you will win if you rate $\frac{2}{3}X$.
  - But wait, now other players will reason through the same argument and rate the model $\frac{2}{3}X$.
  - This means you should rate $\left(\frac{2}{3}\right)^2 X$
  - This argument continues down the road and we reach at a conclusion that we should rate model 1. If any player rates the model more that 1, he will immediately diverge from 2/3'rd of the average and lose.
...
This action profile (of each player rating the mode 1) is known as the `Nash Equilibrium`. Some properties of Nash Equilibrium are:
  ...
  - Each player's action maximizes his/her expected payoff, given the actions of the others.
...
{: .box-warning}
**Note**: Nash equilibrium implicitly assumes everyone plays the action that maximizes *their* utility but if that profile isn't played, nash equilibria fails to provide the best response to that profile of actions.
...
{: .box-note}
**Nash Equlibrium (Definition)**: Action profile $a = \langle a_1, ..., a_n \rangle$ is a *pure strategy* Nash Equilibrium iff $\forall i, a_i \in \mathcal{BR}(a_{-i})$
...
The best move for the Player 1 is to choose D irrespective of what the other player chooses because he is bound to receive either 0 or -3 unlike the case in which he chooses C where he may end up receiving -4! The same goes for player 2 and hence there is a unique nash equilibrium which is the action profile $\\{D, D\\}$. If any one of them deviates from this profile, he/she will end up receiving -4 while the other player receives 0.

2. A couple going to a movie!
...
{: .box-note}
**Strictly Dominant Strategy (Definition)**: If $u\_i(s\_i, S\_{-i}) > u\_i(s'\_{i}, S\_{-i})$ then $s\_i$ strictly dominates $s'\_{i}$

{: .box-note}
**Very Weakly Dominant Strategy (Definition)**: If $u\_i(s\_i, S\_{-i}) \ge u\_i(s'\_{i}, S\_{-i})$ then $s\_i$ very weakly dominates $s'\_{i}$

If one strategy dominates all others, then it is `dominant`.

A strategy profile where everyone is playing a dominant strategy must be a Nash Equilibrium. An equilibrium of strictly dominant strategies must be unique.
...
The dominant strategy profile in this game is $\\{D, D\\}$ because no matter what the other player chooses, choosing D will lead to a "strictly" better outcome.
...
{: .box-note}
From the point of view of an outsider, can some outcomes of a game be said to be `better` than others?
...
So, why is "**Prisoner's *Dilemma***" such a "dilemma"?
